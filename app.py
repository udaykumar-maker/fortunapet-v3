...
@app.route('/customers/bulk-delete', methods=['POST'])
@login_required
def bulk_delete_customers():
    ids = request.form.getlist('customer_ids')
    if not ids:
        flash("No customers selected.", "danger"); return redirect(url_for('customers'))
    n = 0
    for cid in ids:
        c = Customer.query.get(int(cid))
        if c and (current_user.is_admin() or c.created_by_id == current_user.id):
            db.session.delete(c); n += 1
    db.session.commit()
    flash(f"{n} customer(s) deleted.", "success")
    return redirect(url_for('customers'))
...
def edit_item(item_id):
    ...
    it.name          = request.form.get('name', it.name).strip() or it.name
    it.default_uom   = request.form.get('default_uom', it.default_uom)
    it.default_price = float(request.form.get('default_price') or 0)
    db.session.commit()
    flash(f"Item '{it.name}' updated.", "success")
    return redirect(url_for('items'))


@app.route('/pi/bulk-status', methods=['POST'])
@login_required
def bulk_status_update():
    ids    = request.form.getlist('doc_ids')
    status = request.form.get('bulk_status')
    if not ids or not status:
        flash("Select PIs and a status.", "danger"); return redirect(url_for('documents'))
    n = 0
    for did in ids:
        d = Document.query.get(int(did))
        if d and (current_user.is_admin() or d.created_by_id == current_user.id):
            d.status = status; n += 1
    db.session.commit()
    flash(f"{n} PI(s) updated to '{STATUS_LABELS.get(status, status)}'.", "success")
    return redirect(url_for('documents'))


@app.route('/pi/<int:doc_id>/print')
@login_required
def print_pi(doc_id):
    doc = Document.query.get_or_404(doc_id)
    if not current_user.is_admin() and doc.created_by_id != current_user.id:
        flash("Access denied.", "danger"); return redirect(url_for('documents'))
    return render_template('print_pi.html', doc=doc,
        COMPANY_NAME="Nishant Mouldings Pvt Ltd.",
        COMPANY_ADDRESS="#2, Eralinganna Indl Estate, Srigandakaval, Vishwaneedam Post, Sunkadakatte, Bangalore-560 091.",
        COMPANY_URL="www.fortunapet.com", COMPANY_EMAIL="info@fortunapet.com",
        TERMS=["100% Advance payment","Prices - As per the Price increase in the Raw Material."],
        BANK={"Beneficiary":"NISHANT MOULDINGS PVT LTD","Bank":"SBI BANK",
              "A/c No":"41107420952","IFSC":"SBIN0008577","Branch":"Kumara Park"})
...
def api_monthly_sales():
    ...
    for i in range(5, -1, -1):
        ...
        docs = visible_docs().filter(
            db.extract('year',  Document.doc_date) == y,
            db.extract('month', Document.doc_date) == m,
        ).all()
        data.append({
            'label': f"{calendar.month_abbr[m]} {y}",
            'total':    round(sum(d.total_amount or 0 for d in docs), 0),
            'delivered': round(sum(d.total_amount or 0 for d in docs if d.status=='delivered'), 0),
        })
    return jsonify(data)


@app.route('/api/charts/top-customers')
@login_required
def api_top_customers():
    custs = visible_custs().all()
    data  = []
    for c in custs:
        delivered = sum(d.total_amount or 0 for d in c.documents if d.status=='delivered')
        if delivered > 0:
            data.append({'name': c.name[:25], 'delivered': round(delivered, 0)})
    data.sort(key=lambda x: x['delivered'], reverse=True)
    return jsonify(data[:10])


@app.route('/api/charts/top-items')
@login_required
def api_top_items():
    docs  = visible_docs().filter(Document.status=='delivered').all()
    items_map = {}
    for d in docs:
        for li in d.line_items:
            name = li.item_desc or 'Unknown'
            items_map[name] = items_map.get(name, 0) + (li.line_total or 0)
        if not d.line_items and d.item_desc:
            items_map[d.item_desc] = items_map.get(d.item_desc, 0) + (d.total_amount or 0)
    data = [{'name': k[:30], 'value': round(v, 0)} for k,v in items_map.items()]
